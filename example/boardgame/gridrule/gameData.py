...
SymbolTagSignals = GenericSignal[_PtType_, str]
...
class GridData:
    ...
    def __init__(self, name = '棋盘', matr = None, over = False,
        ...
        self.name = name                    # 名称
        self.matr = matr or MatrixP((3,3))      # 坐标对象
...
class PieceData:
    ...
    def __init__(self, **kwargs):
        ...
        self.player = kwargs.get('player', None)        # 棋手
    ...
    def duplicate(self, player = None, **kwargs):
        """获取副本"""
        new_kwargs = {key: getattr(self, key) for key in ('value', 'name', 
                    'count', 'placeable', 'movable', 'occupy', 'squeeze')}
        new_kwargs.update(kwargs)
        new_piece = self.__class__(player or self.player, **new_kwargs)
        return new_piece
...
class PlayerData:
    ...
    def __init__(self, **kwargs):
        self.pieces: dict[int, PieceData] = kwargs.get('pieces', {})    # 棋子 值：棋子
...
class MoveManager:
    ...
    def __init__(self, game: 'GameData'):
        ...
        self.symbol_signals = SymbolTagSignals()
        # 默认参数
        self.step_func = {}
        self.reverse_func = {}
        self.symbols_tag = {'1': [], 'A': []}
        self.init_data()
        self.step_func.setdefault('game_over', self.step_game_over)
        self.reverse_func.setdefault('game_over', self.reverse_game_over)
        self.step_func.setdefault('pass', self.step_pass)
        self.reverse_func.setdefault('pass', self.reverse_pass)
    ...
    def agree_retract(self, askname):
        ...
        for player, node_data in self.grid.history.retract(askname):
    ...
    def move_over(self, player, tag, *data):
        """记录行棋"""
        self.grid.history.move_step(player.name, (tag, data))
    
    def add_move(self, player, tag, *data):
        """添加行棋"""
        self.grid.history.add_move(player.name, (tag, data))

    def move_symbol_tag(self, pt, tag):
        """添加标记"""
        if tag == '1':
            if not self.symbols_tag['1']:
                current = 1
            else:
                current = self.symbols_tag['1'][-1] + 1
            self.symbols_tag['1'].append(current)
            _tag = str(current)
        elif tag == 'A':
            if not self.symbols_tag['A']:
                current = 65
            else:
                current = self.symbols_tag['A'][-1] + 1
                if current > 90:
                    current = 97
            self.symbols_tag['A'].append(current)
            _tag = chr(current)
        else:
            _tag = tag
        if tag == 'remove':
            for p,t1,t2 in self.grid.history.current_symbol:
                if pt == p:
                    self.grid.history.current_symbol.remove((p, t1, t2))
                    if t1 == 'A' and (t := ord(t2)) in self.symbols_tag['A']:
                        self.symbols_tag['A'].remove(t)
                    elif t1 == '1' and (t := int(t2)) in self.symbols_tag['1']:
                        self.symbols_tag['1'].remove(t)
                    self.symbol_signals.call('remove_symbol', pt, _tag)
        else:
            self.symbol_signals.call('update_symbol', pt, _tag)
            self.grid.history.add_symbol((pt, tag, _tag))

    def step_back(self)-> tuple[str, MoveIndexNode]:
        """返回上一步。返回上一步玩家和撤回的数据"""
        prevname, node, node_data, symbol = self.grid.history.back()
    ...
    def step_forward(self)-> tuple[str, MoveIndexNode]:
        ...
        if name:
            for move,data in node_data:
                self.step_func[move](self.game.players[node.player], *data)
    ...
    def call_signal(self, key, *args):
        ...
        elif key in ['update_symbol', 'remove_symbol', 'clear_symbol']:
            self.symbol_signals.call(key, *args)
    ...
    def set_signal(self, key, func):
        if key in ['ask_retract', 'agree_retract',
            ...
            self.symbol_signals.connect(key, func)
    ...
    def _step_pass(self, player):
        """围棋的虚招，或不限手数的游戏结束本回合行棋"""
        if self.grid.history.current_player != player.name and \
                ('pass', tuple()) in self.grid.history.current_data:
            self._step_game_over(player, GameOverEnum.Stop)
    ...
    def _step_change(self, player, val, new_val, pts):
        """绘制棋盘上的棋子"""
        self.step_change(player, new_val, val, pts)
        self.move_over(player, 'change', new_val, val, pts)

    def step_change(self, player, old_val, val, pts):
        """绘制棋盘上的棋子"""
        self.change_pts_piece(val, pts)
        self.update_tag_pts(player, pts, PieceTagEnum.Change)

    def reverse_change(self, player, old_val, val, pts):
        """绘制棋盘上的棋子"""
        self.change_pts_piece(old_val, pts)

    def _step_game_over(self, player, tag):
        self.step_game_over(player, tag)
        self.move_over(player, 'game_over', tag)

    def step_game_over(self, player, tag):
        self.game.game_over(player, tag)

    def reverse_game_over(self, player, tag):
        if player and tag != GameOverEnum.Going and self.game.in_race:
            self.game.on_race()
...
class PlayerManager:
    ...
    def rebegin(self):
        ...
        self.piece_count.update(self.init_piece_count())
        for player in self.player_group.values():
            player.reset(self.player_temporary.get(player.name, {}), self.piece_count)
...
class GameData:
    ...
    __slots__ = ('grid', 'start_clock', 'in_race',
                 ...
                 'player_str_signals', 'temporary', 'on_tag')
    def __init__(self):
        # 棋盘对象
    ...
    def rebegin(self):
        ...
        self.move_manager.symbols_tag = {'1': [], 'A': []}
        self.set_matr_default_piece_pts()
    ...
    def _get_game_data(self, current = True):
        """保存游戏"""
        data = {}
        data['matr'] = self.grid.matr.array2d
        data['over'] = self.grid.over
        if current:
            data['history'] = self.grid.history.simplify_history().to_json()
        else:
            data['history'] = self.grid.history.to_json()
        data['grid_temporary'] = self.grid.temporary
    ...
    def load_data(self, data):
        self.grid.matr.set_array2d(data['matr'])
        self.grid.over = data['over']
        self.grid.history = History.from_json(data['history'], False)
        self.grid.temporary = data['grid_temporary']
    ...
    def move_point(self, pt = None, name = None):
        ...
        if self.on_tag == '':
            ...
            self.move_manager.move_symbol_tag(pt, tag = self.on_tag)
    ...
    def give_up(self, player = None):
        ...
        self.move_manager._step_game_over(player, tag = GameOverEnum.Lose)
    ...
    def call_signal(self, key, *args):
        if key in ['game_begin', 'remove_player', 'give_up']:
            ...
            self.player_str_signals.call(key, *args)
                   ...
                   'remove_tag_pts', 'update_symbol',
                   'remove_symbol', 'clear_symbol']:
    ...
    def call_piece_signal(self, key, *args):
        if key in ['add', 'remove', 'change', 'swap', 'move']:
    ...
    def get_current_path_length_from_branch(self):
        return self.grid.history.get_current_path_length_from_branch()